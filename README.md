...
This repository contains the final calculation pipeline, memo PDF/LaTeX source, output tables, and instructions for obtaining the public HFC/RCTA raw data release.
...
data/raw/         raw-data landing folder; see data/raw/README.md
...
After unzipping, `data/raw/` should contain the three required CSV files listed above. The zip file itself is intentionally ignored by git.
...
The submitted memo is available as `final_memo.pdf` in the repository root. To rebuild the PDF from the LaTeX source, install XeLaTeX and run from the repository root:
